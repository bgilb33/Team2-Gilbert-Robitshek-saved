...
static void udp_client_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "Socket created, sending to %s:%d", HOST_IP_ADDR, PORT);
            ...
            printf("timer_state is: %d\n", timer_state);
            sprintf(outputBuffer, "DATA: " IPSTR ",%d,%f,%d\n", IP2STR(&ip_info.ip),timer_state, temp, step_count);
            printf("Just printed: %s\n", outputBuffer);
...
void app_main()
{
    ...
    init();  // Initialize UART and GPIO pins.

    ESP_ERROR_CHECK(nvs_flash_init());  // Initialize non-volatile storage.

    // Call your custom Wi-Fi connection function here.
    // Ensure that it handles Wi-Fi configuration and connection.
    xTaskCreate(udp_client_task, "udp_client", 4096, NULL, 5, NULL);  // Create UDP client task.
